...
extern uint8_t tx_dma_buffer[8];
void BSP_Console_Init()
{
    ...
    // Enable interrupt on TC event (Transmission Complete)
    ...
    // DMA mode is NOT circular (for RX only)
	...
	// DMA mode is oneshot (not circular)
	DMA1_Channel4->CCR &= ~DMA_CCR_CIRC;
	// Enable DMA1 TC interrupts
	DMA1_Channel4->CCR |= DMA_CCR_TCIE;
